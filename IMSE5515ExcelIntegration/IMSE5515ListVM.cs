...
{
    public class IMSE5515ListVM : INotifyPropertyChanged
    {
        ...
        public List<IMSE5515VM.CellValue> CellValues { get; set; } = new List<IMSE5515VM.CellValue>();

        public Action WriteExcelValues { get; set; }

        public IMSE5515ListVM()
        {
        ...
        private void TakeData()
        {
            try
            {
                if (String.IsNullOrEmpty(DMCList))
                {
                    MessageBox.Show("No DMC set");
                    return;
                }
                var dmcList = DMCList.Split(new string[] { Environment.NewLine }, StringSplitOptions.RemoveEmptyEntries).ToList();
                if (dmcList.Count == 0)
                {
                    MessageBox.Show("No DMC set");
                    return;
                ...
                try
                {
                    var allMeasurementData = REP_D_VJ_REEL_DATALOG.GetTmdbMeasurementsFromList(dmcList);
        ...
        private void FillData()
        {
            ...
            {
                CellValues.Add(new IMSE5515VM.CellValue()
                {
                    CellReference = "A" + (i+1),
                    Result = rows[i]
                });
